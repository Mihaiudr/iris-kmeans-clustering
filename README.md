...
iris_dataset.csv : The raw input data containing only the four features (sepal_length, sepal_width, petal_length, petal_width). This is the dataset used for the unsupervised clustering.

iris_with_answers.csv : The complete dataset, including the actual 'species' labels. This file is used in the final steps of the notebook for validation and performance evaluation.

clustering.yml : A Conda environment file listing all necessary Python packages and their specific versions required to run the project without dependency conflicts.
